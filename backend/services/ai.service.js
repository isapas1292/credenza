...
const AI_SERVICE_URL = (process.env.AI_SERVICE_URL || 'https://credenza-1.onrender.com').replace(/\/+$/, '');

const AiService = {
    async clasificarPerfil(perfil) {
        const response = await axios.post(`${AI_SERVICE_URL}/profile/segment`, { perfil }, { timeout: 60000 });
